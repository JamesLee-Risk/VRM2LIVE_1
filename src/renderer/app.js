...
function buildHotkeyRow(hk) {
  ...
  item.innerHTML = `
    ...
    <div class="small">
      ...
      <label><input type="checkbox" class="hk-global" ${hk.global ? 'checked' : ''} /> 全域</label>
    </div>`;
...
function frame() {
  ...
  if (outputAccumulator >= renderInterval) {
    ...
    api.output.sendState(pose ? { ...resolved, __pose: pose } : resolved);
  }
...
function bindSceneControls() {
  ...
  $('#btn-reset-transform').addEventListener('click', () => {
    ...
    state.modelConfig.transform = { x: 0, y: 0, z: 0, rotY: 0, scale: 1 };
    stage.setModelTransform(state.modelConfig.transform);
    saveModelConfig();
    pushScene();
  });
...
function bindStageInteraction() {
  ...
  canvas.addEventListener('wheel', (e) => {
    ...
    e.preventDefault();
    const tr = state.modelConfig.transform;
    tr.scale = Math.max(0.2, Math.min(4, tr.scale * (e.deltaY < 0 ? 1.06 : 1 / 1.06)));
    stage.setModelTransform(tr);
    saveModelConfig();
  }, { passive: false });
...
async function runSelfTest() {
  ...
  try {
    ...
    if (exprName) {
      ...
      state.modelConfig.hotkeys.push(testHk);
      hotkeyEngine.setHotkeys(allHotkeys());
